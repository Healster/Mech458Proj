...
volatile unsigned int belt_speed = 0x58;
...
void main(int argc,char*argv[])
{
	...
	if (obj_ADC_meas<100) {//add Alum to queue
		initLink(&newLink);
		newLink->e.itemCode = ALUM_BKT;
		enqueue(&head, &tail, &newLink);
	} 
	else if (obj_ADC_meas<=650 && obj_ADC_meas>=100 ) { //add Steel to queue
		initLink(&newLink);
	...
	//OCR0A = 0xF0;//Speed up belt to ensure piece falls when continuing
	...
	intTimer(150);//wait for piece to fall into bin
