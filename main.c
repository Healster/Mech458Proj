...
volatile unsigned int belt_speed = 0x54;
...
void main(int argc,char*argv[])
{
	...
	else if (obj_ADC_meas>650 && obj_ADC_meas<=920) { //add white to queue
		initLink(&newLink);
		newLink->e.itemCode = WHITE_BKT;
		enqueue(&head, &tail, &newLink);
	}
	else if (obj_ADC_meas>920 && obj_ADC_meas<1024) { //add black to queue
		initLink(&newLink);
	...
	OCR0A = 0x70;//Speed up belt to ensure piece falls when continuing
