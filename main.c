...
volatile unsigned int belt_speed = 0xD0;

int delay[11] = {20, 19, 19, 18, 17, 16, 14, 12, 10, 8, 7};
...
int calc_delay(int step, int numSteps);
void main(int argc,char*argv[])
{
	...
	while((PIND&0x04) == 0x04) {//while there is an object in front of laser
		...
		if (ADC_result_flag) {//got adc value
			...
			ADC_calc = (ADC_result + ADC_result_old)/2;//takes average of last and current result
			if (ADC_calc > ADC_result){ //if average is greater, value is decreasing
				ADC_result_old = ADC_result;//update old result to compare it to next new result
			}
		}
	}
	...
	if (obj_ADC_meas<120) {//add Alum to queue
		initLink(&newLink);
		newLink->e.itemCode = ALUM_BKT;
		enqueue(&head, &tail, &newLink);
	} 
	else if (obj_ADC_meas<=650 && obj_ADC_meas>=120 ) { //add Steel to queue
		initLink(&newLink);
		newLink->e.itemCode = STEEL_BKT;
		enqueue(&head, &tail, &newLink);
	}
	else if (obj_ADC_meas>650 && obj_ADC_meas<=910) { //add white to queue
		initLink(&newLink);
		newLink->e.itemCode = WHITE_BKT;
		enqueue(&head, &tail, &newLink);
	}
	else if (obj_ADC_meas>910 && obj_ADC_meas<1024) { //add black to queue
		initLink(&newLink);
	...
	//OCR0A = 0x80;//Speed up belt to ensure piece falls when continuing
	PORTB = CCW;//continue - this will drop item into bin
	//intTimer(60);//wait a sec - this is just for the faster acceleration to shoot piece off
	//OCR0A = belt_speed;//return to normal speed
	intTimer(120);//wait for piece to fall into bin
...
void turn(int numSteps, int dir)
{
	if (numSteps < 50){
		...
		for (int i = 0; i < numSteps; i++) { //i = number of steps taken
			...
			mTimer(calc_delay(i, numSteps)); // Wait for the current delay
...
int calc_delay(int step, int numSteps) {
	...
	int accelSteps = 10;      // Number of steps for acceleration
	int steadySteps = numSteps - (accelSteps*2);     // Number of steps at constant speed

	if (step < accelSteps) {//accel
		return delay[step];
	} 
	else if (step >= accelSteps + steadySteps) {//decel
		return delay[numSteps - step];
	}
	else {
		return delay[accelSteps];
	}
}
...
void PWM () {
	...
	OCR0A = belt_speed; //50% duty cycle
